 ...
 * For each <img data-email="..."> on the page, computes the MD5 hash of the
 * trimmed, lowercased email and sets the src to the corresponding Gravatar URL.
 * Falls back to the mystery-person silhouette if no Gravatar is registered.
 *
 * MD5 implementation based on Paul Johnston's public-domain work.
 */
(function () {
  'use strict';
  ...
  function md5(str) {
    ...
    var ml = ((l + 8) >> 6 << 4) + 14;
    var m = new Array(ml + 1);
    for (var i = 0; i <= ml; i++) m[i] = 0;
    for (i = 0; i < l; i++) m[i >> 2] |= s.charCodeAt(i) << (i % 4 << 3);
    m[l >> 2] |= 0x80 << (l % 4 << 3);
    m[ml] = l * 8;
    ...
    [a, b, c, d].forEach(function (v) {
      for (var k = 0; k < 4; k++) {
        hex += ('0' + ((v >>> (k * 8)) & 0xFF).toString(16)).slice(-2);
      }
    });
    return hex;
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('img[data-email]').forEach(function (img) {
      var email = (img.dataset.email || '').trim().toLowerCase();
      if (email) {
        img.src = 'https://www.gravatar.com/avatar/' + md5(email) + '?s=200&d=mp';
      }
    });
  });
})();
